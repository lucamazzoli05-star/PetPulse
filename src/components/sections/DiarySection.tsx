...
function formatDate(d: string) {
  return new Date(d).toLocaleDateString('it-IT', { weekday: 'short', day: '2-digit', month: 'long', year: 'numeric' })
}
    ...
    <div>
      ...
      {entries.length === 0 ? (
        <EmptyState
          icon={BookOpen}
          title="Diario vuoto"
          description="Scrivi note, osservazioni o ricordi del tuo animale"
          action={{ label: 'Prima nota', onClick: () => setShowModal(true) }}
        />
      ) : (
          ...
          {entries.map(e => (
            <div key={e.id} className="bg-white rounded-xl border border-gray-100 p-3">
              <div className="flex items-start justify-between gap-2">
                <p className="text-xs text-primary-600 font-medium">{formatDate(e.date)}</p>
      ...
      {showModal && (
        <Modal title="Nuova nota diario" onClose={() => setShowModal(false)}>
